...
class ProcessResult:
    ...
    chl_image: np.ndarray         # [H, W, 3] uint8
    cal_image: np.ndarray         # [H, W, 3] uint8
    lon: np.ndarray               # [H, W]
    lat: np.ndarray               # [H, W]
...
class MicaEngine:
    ...
    def __init__(
        ...
        *,
        ...
        """
        Facade over :class:`cramm.classifier.MicaClassifier` +
        :class:`cramm.renderer.ResultRenderer`.

        Parameters
        ----------
        reference_track : ignored
            Kept for API compatibility; the classifier always runs the 1 nm
            rule-face track.
        """
        self.rf_path = Path(rf_path) if rf_path else None  # None → built-in rule library
        self.splib_path = Path(splib_path) if splib_path else None  # None → bundled cramm/data/rf77_splib07_1nm.npz
        ...
        self._classifier = MicaClassifier.from_paths(
            rf_path, splib_path, reference_track=reference_track)
    ...
    def get_resample(self, w: np.ndarray, bp: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        Keys are rule-name strings (the sole link to the reference spectra
        library after the v1.4.0 id purge); values are float64 resampled
        spectra indexed by EMIT band.
        """
    ...
    def spectrum_analysis(
        ...
        raw : bool, optional
            False (default) returns the five uint8 images
            (orth_rgb, color_enhanced, mus_image, chl_image, cal_image).
            True additionally returns four float arrays:
              mus_center — muscovite 2.2μm absorption center wavelength (μm);
                           non-muscovite pixels are 0.
              chl_center — chlorite 2250nm absorption center wavelength (μm);
                           non-chlorite pixels are 0.
              cal_center — carbonate 2330nm absorption center wavelength (μm);
                           non-carbonate pixels are 0.
              fd — per-pixel best (argmax) fit×depth joint confidence.

        Returns
        -------
        orth_rgb, color_enhanced, mus_image, chl_image, cal_image : np.ndarray
            All [r, c, 3] uint8.
        mus_center : np.ndarray, optional (when raw=True)
            [r, c] float, muscovite 2200nm absorption wavelength (μm).
        chl_center : np.ndarray, optional (when raw=True)
            [r, c] float, chlorite 2250nm absorption wavelength (μm).
        cal_center : np.ndarray, optional (when raw=True)
            [r, c] float, carbonate 2330nm absorption wavelength (μm).
        ...
        orth_rgb, color_enhanced, mus_image, chl_image, cal_image = self._renderer.render(result)
        if raw:
            return (
                orth_rgb, color_enhanced, mus_image, chl_image, cal_image,
                result.mus_center.reshape(result.r, result.c),
                result.chl_center.reshape(result.r, result.c),
                result.cal_center.reshape(result.r, result.c),
                result.depth.reshape(result.r, result.c),
            )
        return orth_rgb, color_enhanced, mus_image, chl_image, cal_image
    ...
    def write_tiff(
        ...
        chl_image: Optional[np.ndarray] = None,
        cal_image: Optional[np.ndarray] = None,
    ) -> None:
        """Handle coordinate resampling and GeoTIFF file saving (delegated to ResultRenderer)."""
        ResultRenderer.write_tiff(
            pathname, lon, lat, orth_rgb, color_enhanced, mus_image, chl_image, cal_image
        )
...
def main():
    """Command-line entry point: process EMIT NetCDF → 5 GeoTIFFs + float variables."""
    import argparse
    import time
    ...
    parser.add_argument("--raw", action="store_true", help="also save mus_center + chl_center + cal_center + fd float arrays as npz")
    ...
    t0 = time.time()
    if args.raw:
        orth, color, mus, chl, cal, mus_center, chl_center, cal_center, fd = engine.spectrum_analysis(
        ...
        np.savez_compressed(
            ...
            mus_center=mus_center, chl_center=chl_center, cal_center=cal_center, fd=fd,
        )
        print(f"[CRAMM] Raw data saved: {out_name}_raw.npz", flush=True)
    else:
        orth, color, mus, chl, cal = engine.spectrum_analysis(
            spectrum, wl, w, bp, chanels, n_workers=args.workers,
            progress_callback=lambda p: print(f"[CRAMM] {p}%", flush=True),
        )
    print(f"[CRAMM] Analysis: {time.time()-t0:.1f}s", flush=True)

    engine.write_tiff(out_path, lon, lat, orth, color, mus, chl, cal)
    print(f"[CRAMM] GeoTIFF written to: {out_dir}", flush=True)
    for suffix in ["_mapping_orth.tiff", "_color_enhanced_orth.tiff", "_mus_orth.tiff", "_chl_orth.tiff", "_cal_orth.tiff"]:
        p = out_path + suffix
        if os.path.exists(p):
